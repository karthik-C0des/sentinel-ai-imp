    ...
    return (
      <div style={{ marginTop: spacing[3] }}>
        ...
        <div style={{
          ...
          {/* Error Message */}
          {components.error && (() => {
            const err = components.error;
            const isBedrockAccess = err.includes('ValidationException') || err.includes('AccessDeniedException') || err.includes('not authorized') || err.includes('Operation not allowed');
            return (
              <div style={{ marginBottom: spacing[2], padding: '10px 14px', backgroundColor: palette.yellow.light3, borderRadius: '6px', border: `1px solid ${palette.yellow.base}` }}>
                <Body weight="medium" style={{ color: palette.yellow.dark2 }}>⚠ Vector Search Unavailable</Body>
                <Body size="small" style={{ color: palette.gray.dark2, marginTop: spacing[1] }}>
                  {isBedrockAccess
                    ? 'AWS Bedrock model access is not enabled for this account. To enable it, go to the AWS Bedrock Console → Model Access and enable "Titan Embeddings G1 - Text". Vector similarity scoring will be skipped until access is granted.'
                    : `Vector search error: ${err}`}
                </Body>
              </div>
            );
          })()}
      ...
      <ExpandableCard
        title={
          <div style={{ display: 'flex', alignItems: 'center', gap: spacing[2] }}>
            <Icon glyph="InfoWithCircle" fill={methodColor} />
            <div>
