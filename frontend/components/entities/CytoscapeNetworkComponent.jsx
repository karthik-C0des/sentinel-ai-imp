...
// cytoscape is loaded dynamically inside useEffect to prevent SSR issues
// (Cytoscape.js requires a DOM environment; static import breaks Next.js SSR)
...
const CytoscapeNetworkComponent = ({ 
  ...
  useEffect(() => {
    const loadExtensions = async () => {
      if (!extensionsLoaded) {
        // Load cytoscape core first if not already loaded
        if (!cytoscapeLib) {
        ...
        if (!contextMenus) {
          ...
          cytoscapeLib.use(contextMenus);
        }
        if (!panzoom) {
          panzoom = (await import('cytoscape-panzoom')).default;
          cytoscapeLib.use(panzoom);
        }
