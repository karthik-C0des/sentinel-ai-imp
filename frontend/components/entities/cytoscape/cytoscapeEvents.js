...
export const setupCytoscapeEvents = (cy, callbacks = {}) => {
  ...
  cy.on('mouseover', 'edge', function(evt) {
    ...
    if (!edge.target().removed()) edge.target().addClass('highlighted');
  ...
  cy.on('mouseout', 'edge', function(evt) {
    ...
    if (!edge.selected()) {
      ...
      if (!edge.source().removed()) edge.source().removeClass('highlighted');
      if (!edge.target().removed()) edge.target().removeClass('highlighted');
    }
...
const getRelationshipType = (node1, node2) => {
  const edge = node1.edgesWith(node2)[0];
  return edge ? edge.data('relationshipType') || edge.data('label') || edge.data('type') || 'Unknown' : 'None';
};

// Calculate relationship strength based on edge data
const calculateRelationshipStrength = (edge) => {
  return edge.data('strength') || edge.data('weight') || edge.data('amount') ? 0.8 : 0.5;
};

// Calculate how risk propagates across an edge
const calculateRiskPropagation = (edge) => {
  const sourceRisk = edge.source().data('riskScore') || 0;
  const targetRisk = edge.target().data('riskScore') || 0;
  const strength = calculateRelationshipStrength(edge);
  return ((sourceRisk + targetRisk) / 2) * strength;
};

// Check if an edge is part of a suspicious path
const checkSuspiciousPath = (cy, edge) => {
  const sourceRisk = edge.source().data('riskScore') || 0;
  const targetRisk = edge.target().data('riskScore') || 0;
  return sourceRisk > 60 && targetRisk > 60;
};

// Calculate edge length visually
...
const findAlternativePaths = (cy, source, target) => {
  let common = 0;
  const targetNeighbors = new Set(target.neighborhood().nodes().map(n => n.id()));
  source.neighborhood().nodes().forEach(n1 => {
    if (n1.id() !== target.id() && targetNeighbors.has(n1.id())) {
      common++;
    }
  });
  return common;
...
// Check if edge bridges two distinct communities
const isCommunityBridge = (cy, edge) => {
  const sourceComm = edge.source().data('community') || edge.source().data('cluster');
  const targetComm = edge.target().data('community') || edge.target().data('cluster');
  return sourceComm && targetComm && sourceComm !== targetComm;
};
