  ...
  return (
    <Card style={{ padding: spacing[4] }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: spacing[3] }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: spacing[1] }}>
          <Icon 
            glyph={resolution.status === 'resolved' ? 'CheckmarkWithCircle' : 'ClockWithArrow'} 
            size={20} 
...
function ComprehensiveOverviewTab({ entity }) {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: spacing[4] }}>
      ...
      <Tabs selected={activeSubTab} setSelected={setActiveSubTab} aria-label="Overview Tabs">
        <Tab name="Identifier">
          ...
          <Card style={{ padding: spacing[4] }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: spacing[3] }}>
              <div>
                ...
                <Body weight="medium">{entity.name?.full || '-'}</Body>
              </div>
              {entity.name?.structured && (
                <>
                  <div>
                    <Label>First Name</Label>
                    <Body weight="medium">{entity.name.structured.first || '-'}</Body>
                  </div>
                  <div>
                    <Label>Last Name</Label>
                    <Body weight="medium">{entity.name.structured.last || '-'}</Body>
                  </div>
                </>
              )}
              {entity.dateOfBirth && (
              ...
              {entity.addresses.map((address, index) => (
                <div key={index} style={{ 
                  ...
                  <div style={{ marginTop: spacing[2] }}>
                    ...
                    <Body weight="medium">{address.full || '-'}</Body>
                  </div>
                ...
                <TableBody>
                  {entity.identifiers.map((identifier, index) => (
                    <Row key={index}>
                      ...
                      <Cell>
                        <Body weight="medium">{identifier.value || '-'}</Body>
                      </Cell>
                      <Cell>
                        <Body>{identifier.country || '-'}</Body>
                      </Cell>
                      <Cell>
                      ...
                      <Cell>
                        <Body style={{ 
                          color: identifier.verified === true ? palette.green.dark2 : identifier.verified === false ? palette.red.base : palette.gray.dark1 
                        }}>
                          {identifier.verified === true ? 'Yes' : identifier.verified === false ? 'No' : '-'}
                        </Body>
                      </Cell>
                    </Row>
                  ))}
          ...
          <div style={{ marginTop: spacing[3] }}>
            ...
            <div style={{ marginTop: spacing[4] }}>
              <div style={{ 
                ...
                <Icon glyph="ActivityFeed" size={20} fill={palette.green.base} />
          ...
          {entity.behavioral_analytics && (
            <Card style={{ padding: spacing[4] }}>
              <H3 style={{ marginBottom: spacing[3] }}>
                <Icon glyph="ActivityFeed" style={{ marginRight: spacing[2] }} />
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: spacing[3] }}>
                      <div>
                        ...
                        <Body weight="medium" style={{ fontSize: '18px', color: palette.blue.dark2 }}>
                          ${entity.behavioral_analytics.transaction_patterns.avg_transaction_amount?.toFixed(2) || '-'}
                        </Body>
                      </div>
                      <div>
                        <Label>Standard Deviation</Label>
                        <Body weight="medium" style={{ fontSize: '18px', color: palette.gray.dark2 }}>
                          ${entity.behavioral_analytics.transaction_patterns.std_transaction_amount?.toFixed(2) || '-'}
                        </Body>
                      </div>
                      <div>
                        <Label>Avg Transactions Per Day</Label>
                        <Body weight="medium" style={{ fontSize: '18px', color: palette.gray.dark2 }}>
                          {entity.behavioral_analytics.transaction_patterns.avg_transactions_per_day?.toFixed(2) || '-'}
                        </Body>
                      </div>
                      ...
                      <TableBody>
                        {entity.behavioral_analytics.location_patterns.map((location, index) => {
                          ...
                          return (
                            <Row key={index}>
                              <Cell>
                                <Body weight="medium">{location.city || '-'}</Body>
                              </Cell>
                              <Cell>
                                <Body>{location.state || '-'}</Body>
                              </Cell>
                              <Cell>
                                <Body>{location.country || '-'}</Body>
                              </Cell>
                              <Cell>
  ...
  return (
    <div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 4fr', gap: spacing[4] }}>
        ...
        {/* Right Panel - Tabs */}
        <div>
          <Tabs selected={activeTab} setSelected={setActiveTab} aria-label="Entity Details Tabs">
            <Tab name="Overview">
              <div style={{ marginTop: spacing[3] }}>
