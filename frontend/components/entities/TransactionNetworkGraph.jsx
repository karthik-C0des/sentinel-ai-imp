...
// cytoscape and its extensions are loaded dynamically to prevent SSR issues
// (Cytoscape.js requires a DOM environment; static imports break Next.js SSR)
...
const TransactionNetworkGraph = ({ entityId, onError }) => {
  ...
  // Load Cytoscape core + extensions dynamically (client-only, prevents SSR issues)
  useEffect(() => {
    const loadCytoscape = async () => {
      ...
      if (!dagreExt) {
        dagreExt = (await import('cytoscape-dagre')).default;
        cytoscapeLib.use(dagreExt);
      }
      if (!fcoseExt) {
        fcoseExt = (await import('cytoscape-fcose')).default;
        cytoscapeLib.use(fcoseExt);
      }
  ...
  useEffect(() => {
    if (!containerRef.current || !cytoscapeElements.length || !extensionsLoaded) return;
  ...
  }, [cytoscapeElements, extensionsLoaded, containerRef.current]);
          ...
          <div 
            ref={(el) => {
              if (el && showFullscreenModal && cytoscapeElements.length > 0) {
                ...
                fullscreenCyRef.current = cytoscapeLib({
                  container: el,
                  elements: cytoscapeElements,
                  style: transactionNetworkStyles,
