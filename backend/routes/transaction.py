...
async def evaluate_transaction(
    ...
    if is_unusual:
        ...
        other_risk = [t for t in similar_transactions if t.get("risk_assessment", {}).get("level") not in ["high", "medium", "low"]]
        
        # Build display list prioritizing medium and high risk
        display_transactions = high_risk + medium_risk + low_risk + other_risk
    else:
        ...
        display_transactions = low_risk + medium_risk + high_risk + other_risk
    ...
    if display_transactions:
        ...
        for idx, t in enumerate(display_transactions):
            ...
            # Get risk information — use a loop-local name to avoid shadowing the
            # outer `risk_assessment` that belongs to the current transaction.
            similar_risk = t.get("risk_assessment", {})
            risk_level = similar_risk.get("level", "unknown")
            risk_score = similar_risk.get("score", 50) / 100.0  # Normalize to 0-1 range
            risk_flags = similar_risk.get("flags", [])
