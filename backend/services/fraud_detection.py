...
class FraudDetectionService:
    ...
    def __init__(self, db_client: MongoDBAccess, db_name: str = None, customer_db_name: str = None):
        """
    ...
    async def find_similar_transactions(self, transaction: Dict[str, Any]) -> Tuple[List[Dict[str, Any]], float, Dict]:
        ...
        try:
            ...
            try:
                # Generate embedding FIRST — the pipeline references it, so it must exist
                # before the pipeline dict is constructed. If Bedrock is unavailable this
                # raises immediately and we fall through to the field-based fallback below.
                transaction_embedding = await get_embedding(transaction_text)

                # Build the vector-search pipeline only after the embedding is ready
                pipeline = [
                    {
                        "$vectorSearch": {
                ...
                logger.warning(f"Vector search unavailable ({type(embed_err).__name__}), using field-based fallback")
                
                # ── Field-based fallback: match on merchant category + transaction type ──
                # Build a query that finds transactions similar in key behavioral features
                fallback_query: Dict[str, Any] = {}
                
                if merchant_category and merchant_category != "unknown":
                    fallback_query["merchant.category"] = merchant_category
                if transaction_type and transaction_type != "unknown":
                    fallback_query["type"] = transaction_type
                
                # Amount within 3x range to find contextually similar transactions
                if amount and amount > 0:
                    fallback_query["amount"] = {"$gte": amount * 0.1, "$lte": amount * 10}
                
                # Fetch up to 15 matching transactions, newest first
                raw_docs = list(
                    collection.find(fallback_query, {
                        "_id": 1, "transaction_id": 1, "timestamp": 1, "amount": 1,
                        "merchant": 1, "type": 1, "payment_method": 1,
                        "risk_assessment": 1
                    })
                    .sort("timestamp", -1)
                    .limit(15)
                ...
                # Normalise field names to match what vector search returns
                similar_transactions = []
                for doc in raw_docs:
                    doc["transaction_type"] = doc.pop("type", None)
                    # Assign a synthetic similarity score based on field overlap
                    match_score = 0.5  # base
                    if doc.get("merchant", {}).get("category") == merchant_category:
                        match_score += 0.25
                    if doc.get("transaction_type") == transaction_type:
                        match_score += 0.15
                    if amount and doc.get("amount", 0) > 0:
                        ratio = min(amount, doc["amount"]) / max(amount, doc["amount"])
                        match_score += ratio * 0.1
                    doc["score"] = round(min(match_score, 1.0), 4)
                    similar_transactions.append(doc)
                
                logger.info(f"Field-based fallback found {len(similar_transactions)} transactions")
