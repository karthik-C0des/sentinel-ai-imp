...
async def setup():
    ...
    print("\n1. Creating entity_resolution_text_search (Text Index)")
    try:
        text_index = SearchIndexModel(
            definition={"mappings": {"dynamic": True}},
            name="entity_resolution_text_search",
            type="search"
        )
        await collection.create_search_index(text_index)
        print("Successfully initiated entity_resolution_text_search index creation")
    except Exception as e:
        print(f"Text index creation returned: {e}")

    print("\n2. Creating entity_vector_search_384_new (Vector Index)")
    try:
        vector_index = SearchIndexModel(
            ...
            name="entity_vector_search_384_new",
            type="vectorSearch"
        )
        await collection.create_search_index(vector_index)
        print("Successfully initiated entity_vector_search_384_new index creation")
    except Exception as e:
