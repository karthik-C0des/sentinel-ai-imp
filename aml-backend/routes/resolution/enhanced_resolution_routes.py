...
async def perform_network_analysis(
    ...
    try:
        ...
        from models.api.requests import EntityNetworkRequest
        
        network_request = EntityNetworkRequest(
            entity_id=entity_ids[0],
            max_depth=analysis_config.get("maxDepth", 2),
            max_entities=analysis_config.get("maxNodes", 100)
        )
        
        # Build network for the matched entities
        network_data_response = await network_service.build_entity_network(network_request)
        network_data = network_data_response.network_data.dict() if network_data_response.success and network_data_response.network_data else {"nodes": [], "edges": []}
        
        # Calculate centrality metrics
        centrality_response = await network_service.analyze_network_centrality(entity_ids)
        centrality_metrics = centrality_response.get("centrality_metrics", {})
        
        # Detect hub entities
        hub_response = await network_service.detect_hub_entities(
            min_connections=2
        )
        hub_entities = hub_response.get("hub_entities", [])
        
        # Risk propagation analysis
        risk_response = await network_service.calculate_network_risk_score(
            entity_id=entity_ids[0],
            analysis_depth=2
        )
        risk_propagation = risk_response.get("risk_analysis", {})
