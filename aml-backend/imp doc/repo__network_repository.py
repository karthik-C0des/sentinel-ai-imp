...
class NetworkRepository(NetworkRepositoryInterface):
    ...
    def __init__(self, mongodb_repo: MongoDBRepository, 
                 entity_collection: str = "sentinelaiEntities",
                 relationship_collection: str = "sentinelaiRelationships"):
    ...
    async def build_entity_network(self, params: NetworkQueryParams) -> NetworkDataResponse:
        ...
        try:
            ...
            nodes = []
            edges = []
            entity_ids = {params.center_entity_id}
            ...
            stats_results = await self.repo.execute_pipeline("sentinelaiEntities", stats_pipeline)
            ...
            if relationship_ids:
                ...
                if valid_object_ids:
                    ...
                    rel_dist_results = await self.repo.execute_pipeline("sentinelaiRelationships", relationship_dist_pipeline)
                else:
                    rel_dist_results = []
            else:
                rel_dist_results = []
    ...
    async def find_relationship_path(self, source_entity_id: str,
        ...
        try:
            ...
            results = await self.repo.execute_pipeline("sentinelaiEntities", pipeline)
            ...
            path_results = await self.repo.execute_pipeline("sentinelaiEntities", path_pipeline)
    ...
    async def _build_network_graph(self, params: NetworkQueryParams) -> Dict[str, Any]:
        ...
        try:
            ...
            if restrict_conditions:
                ...
                relationships = await self.repo.execute_pipeline("sentinelaiEntities", manual_pipeline)
            else:
                # Use fluent interface when no complex filters
                relationships = await self.repo.execute_pipeline("sentinelaiEntities", pipeline)
