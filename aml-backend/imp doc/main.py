...
# Import Re-KYC routes
try:
    from routes.rekyc_routes import router as rekyc_router
except ImportError:
    logger.warning("Re-KYC routes not available")
    rekyc_router = None
...
async def startup_event():
    ...
    logger.info("=" * 80)
    logger.info("SentinelAI - AML/KYC API Starting...")
...
# 4.11. Re-KYC Periodic Review routes (RBI KYC Master Direction)
include_router_safely(app, rekyc_router, "Re-KYC Periodic Review")
...
if __name__ == "__main__":
    ...
    logger.info(f"Starting SentinelAI AML/KYC API on {host}:{port}")
