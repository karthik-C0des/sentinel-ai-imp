...
def _compute_degree_centrality(db, entity_id: str, network_size: int) -> float:
    ...
    out_degree = db["sentinelaiRelationships"].count_documents({"source.entityId": entity_id})
    in_degree = db["sentinelaiRelationships"].count_documents({"target.entityId": entity_id})
...
def _compute_network_risk(db, entity_id: str, suspicious_connections: list) -> dict:
    ...
    connected_entities = list(db["sentinelaiEntities"].find(
