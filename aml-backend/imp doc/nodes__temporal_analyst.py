...
Thresholds (INR - India jurisdiction):
  CTR_THRESHOLD  : Rs.10,00,000 (Rs.10 Lakhs) -- PMLA Cash Transaction Report
  PAN_THRESHOLD  : Rs.50,000                   -- Income Tax Rule 114B (PAN required)
  PAN_LOWER      : Rs.48,000                   -- PAN structuring window lower bound

Override via environment variables:
  STRUCTURING_CTR_THRESHOLD  (default 1000000)
  STRUCTURING_PAN_THRESHOLD  (default 50000)
  STRUCTURING_PAN_LOWER      (default 48000)
"""
...
# ── Indian jurisdiction thresholds (INR) ──────────────────────────────
# PMLA Cash Transaction Report: Rs.10 Lakhs (10,00,000)
CTR_THRESHOLD: int = int(os.getenv("STRUCTURING_CTR_THRESHOLD", "1000000"))
# Income Tax Rule 114B PAN structuring band: Rs.48,000 to Rs.49,999
PAN_THRESHOLD: int = int(os.getenv("STRUCTURING_PAN_THRESHOLD", "50000"))
PAN_LOWER: int = int(os.getenv("STRUCTURING_PAN_LOWER", "48000"))

# Legacy alias kept for backward compatibility
STRUCTURING_THRESHOLD = CTR_THRESHOLD
STRUCTURING_LOWER = PAN_LOWER

DORMANCY_GAP_DAYS = 30
VELOCITY_ZSCORE_THRESHOLD = 2.0
JURISDICTION = os.getenv("JURISDICTION", "IN")  # IN = India
...
def _detect_velocity_anomalies(db, entity_id: str) -> list[dict]:
    ...
    pipeline = [
        ...
        {"$addFields": {
            "week": {"$dateToString": {"format": "%Y-W%V", "date": {"$toDate": "$timestamp"}}},
        }},
        {"$group": {
            "_id": "$week",
            "count": {"$sum": 1},
        ...
        {"$lookup": {
            ...
            "pipeline": [
                {"$match": {"$expr": {"$and": [
                    ...
                    {"$gt": [{"$toDate": "$timestamp"}, {"$toDate": "$$outgoing_ts"}]},
                ]}}},
                {"$limit": 1},
            ],
            "as": "return_txn",
...
def _detect_time_anomalies(db, entity_id: str) -> list[dict]:
    ...
    pipeline = [
        ...
        {"$addFields": {
            "parsed_date": {"$toDate": "$timestamp"}
        }},
        {"$addFields": {
            "hour": {"$hour": "$parsed_date"},
            "day_of_week": {"$dayOfWeek": "$parsed_date"},
            "is_off_hours": {"$or": [
                {"$lt": [{"$hour": "$parsed_date"}, 6]},
                {"$gte": [{"$hour": "$parsed_date"}, 22]},
            ]},
            "is_weekend": {"$in": [{"$dayOfWeek": "$parsed_date"}, [1, 7]]},
        }},
        {"$facet": {
            "totals": [
                {"$group": {
...
def _detect_dormancy_bursts(db, entity_id: str) -> list[dict]:
    ...
    pipeline = [
        ...
        {"$group": {
            "_id": None,
            "timestamps": {"$push": {"$toDate": "$timestamp"}},
            "amounts": {"$push": "$amount"},
